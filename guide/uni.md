...
### 4、开启了辅助IP自动化配置，为什么重启网络服务后，网络不通？

开启辅助IP自动化配置后，在您重启主机时，会触发配置文件，配置文件会对网卡所绑定的IP，进行自动化配置，但并未做网络持久化，如你仅针对主机内的网络触发重启，可能会导致路由丢失，此时会导致您的网络出现问题。解决方案是，重新执行*sudo cloud-init init*命令，或者进行持久化路由配置，以 ubuntu20.04版本 为例：需要在 /etc/netplan/50-cloud-init.yaml文件里面,持久化路由配置，即新增

```
   routes:
    \- to: 0.0.0.0/0
     via: 10.35.0.1  # 此为网关
     from: 152.32.220.196 # 此为网络出口
    \- to: 100.64.0.0/10
     via: 10.35.0.1  
    \- to: 192.168.0.0/16
     via: 10.35.0.1  
...
同时如有 gateway4: 10.35.0.1 ，请注释或者删除  